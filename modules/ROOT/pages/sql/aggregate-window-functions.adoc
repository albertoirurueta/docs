...
:description: Explains standard SQL aggregate functions (COUNT, SUM, AVG, MIN, MAX) and window functions (ROW_NUMBER, RANK, DENSE_RANK, LAG, LEAD, NTILE) with the OVER/PARTITION BY/ORDER BY clause, including using aggregates as window functions.
:keywords: sql, aggregate functions, window functions, count, sum, avg, min, max, over clause, partition by, row_number, rank, dense_rank, lag, lead, ntile
