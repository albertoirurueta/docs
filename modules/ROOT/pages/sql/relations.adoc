...
xref:sql/ddl.adoc[]. The join syntax used in each example is documented in full in
xref:sql/dml-queries.adoc[]. For the authoritative (if less readable) specification, see
...
returned (see xref:sql/dml-queries.adoc#_left_outer_join[] for `LEFT OUTER JOIN` syntax):
...
on the many side. This is exactly the relationship already shown in xref:sql/ddl.adoc[] between
...
The foreign key is already defined in xref:sql/ddl.adoc#_foreign_key[]:
...
Joining an order to its line items (see xref:sql/dml-queries.adoc#_inner_join[] for `INNER JOIN` syntax):
...
xref:sql/dml-queries.adoc#_inner_join[]):
