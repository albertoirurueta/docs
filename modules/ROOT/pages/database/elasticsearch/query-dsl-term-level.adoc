...
Term-level queries find documents by an *exact value* in a structured field -- a keyword, number,
date, boolean, IP or range. Unlike
xref:database/elasticsearch/query-dsl-full-text.adoc[full-text queries], they do *not* run the
search term through an analyzer, so what you pass is matched byte-for-byte against the terms in the
index. See
https://www.elastic.co/guide/en/elasticsearch/reference/current/term-level-queries.html[Term-level queries].
...
the literal `"Wireless Keyboard"` looks for a single term with that exact casing and spacing --
which does not exist -- and matches nothing.
...
xref:database/elasticsearch/mapping-and-field-types.adoc[Mapping & field types] for the `text` vs
`keyword` split, and
https://www.elastic.co/guide/en/elasticsearch/reference/current/query-dsl-term-query.html[`term`].
...
[TIP]
====
`term` on a `keyword` field is case- and whitespace-sensitive. For a case-insensitive exact match
set `"case_insensitive": true` on the `term` query, or map a `normalizer` (a lowercasing analyzer
for `keyword`) on the field. This mirrors the difference between `=` and `ILIKE`/`LOWER(col) =` in
...
https://www.elastic.co/guide/en/elasticsearch/reference/current/query-dsl-range-query.html[`range`]
and
...
`exists` matches documents that have any non-null value for a field. There is no "is null" query --
negate `exists` inside `must_not`. A field is considered missing when it is absent, `null`, or `[]`.
See
...
https://www.elastic.co/guide/en/elasticsearch/reference/current/keyword.html#wildcard-field-type[`wildcard` field type]
...
xref:database/elasticsearch/query-dsl-full-text.adoc[Full-text queries]; to combine term-level
filters with scored clauses, continue with
xref:database/elasticsearch/compound-queries-and-relevance.adoc[Compound queries & relevance].
