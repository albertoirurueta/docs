...
https://www.elastic.co/guide/en/elasticsearch/reference/current/removal-of-types.html[Removal of mapping types]
and the overview at
https://www.elastic.co/guide/en/elasticsearch/reference/current/documents-indices.html[Data in: documents and indices].

== An index is a collection of documents + settings + mappings

An index groups documents that share a purpose. It bundles three things: the documents themselves,
*settings* (how the index is stored and sharded), and *mappings* (the field types). Create all three
in one call:
...
are covered in xref:database/elasticsearch/cluster-nodes-and-shards.adoc[Cluster, nodes & shards],
...
See https://www.elastic.co/guide/en/elasticsearch/reference/current/indices.html[Index APIs],
https://www.elastic.co/guide/en/elasticsearch/reference/current/indices-create-index.html[Create index API],
and
https://www.elastic.co/guide/en/elasticsearch/reference/current/cat-indices.html[cat indices API].
Field types themselves are in
...
A relational `WHERE title LIKE '%fox%'` (see xref:database/sql/dml-queries.adoc[SQL Queries]) has a
...
xref:database/elasticsearch/text-analysis.adoc[Text analysis], and exact/substring-style matching on
unanalyzed values is covered in
xref:database/elasticsearch/query-dsl-term-level.adoc[Term-level queries].

=== Lucene segments

Each shard is a full Lucene index, physically made of *segments*: small, immutable inverted indices.
A new or updated document is written to a fresh segment; deletes are just a tombstone marking a doc
id dead in its segment. A background process merges small segments into larger ones and drops the
tombstoned documents. Because segments are immutable, they can be cached and memory-mapped
aggressively, but a document only becomes searchable once the segment holding it is opened -- which
is the next section. See
...
The default refresh interval is one second, so most applications never notice. When a test or a
workflow needs read-your-write visibility, override it per request:
...
https://www.elastic.co/guide/en/elasticsearch/reference/current/docs-refresh.html[the `?refresh` parameter].
...
xref:database/elasticsearch/performance-tuning.adoc[Performance tuning]. Bulk and CRUD mechanics are
in xref:database/elasticsearch/indexing-crud-and-bulk.adoc[Indexing: CRUD & bulk]; if you are just
