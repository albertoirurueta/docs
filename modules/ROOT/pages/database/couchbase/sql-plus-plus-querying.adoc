...
:description: Reading JSON with SQL++ on Couchbase Server -- the Query service and how to run a statement, the SELECT clause set, RAW/VALUE projections, nested-path and array expressions, UNNEST and NEST, lookup / ANSI / index joins, WITH and recursive CTEs, subqueries and collection operators, and how a SQL++ query executes through the optimizer, index scan and join/filter/group/order pipeline.
:keywords: couchbase, sql++, n1ql, query service, cbq, query workbench, cluster.query, keyspace, USE KEYS, SELECT RAW, UNNEST, NEST, ANSI JOIN, lookup join, index join, WITH, recursive CTE, LET, LETTING, ANY, EVERY, ARRAY, FIRST, query plan, optimizer, index selection, query execution
...
----
-- page 1
SELECT r.id, r.airline, r.distance
FROM `travel-sample`.inventory.route AS r
ORDER BY r.id
LIMIT 20;

-- next page: start after the last id seen, no OFFSET
SELECT r.id, r.airline, r.distance
FROM `travel-sample`.inventory.route AS r
WHERE r.id > $lastKey
ORDER BY r.id
...
pattern applies across SQL, MongoDB, Solr, Elasticsearch, GraphQL and Spring Data.
