...
* `if($condition, $if-true, $if-false)` -- lazily evaluates only the branch it returns, but is itself now
  **deprecated** in favor of the modern CSS `if()` syntax (`if(sass($condition): $if-true; else: $if-false)`);
  see https://sass-lang.com/d/if-function[the deprecation notice].
