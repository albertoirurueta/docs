...
// Correct
$good: math.div(100px, 3);       // 33.33333px
----
...
$g: 1in + 10px;         // 1.1041667in -- compatible units are converted
----
...
Equality is strict about type and unit: `1 == 1px` is `false`, and `"foo" == foo` is `true` (quoting does not
affect string equality), while `1px == 1PX` is `true` for units but `"a" == "A"` is `false` for strings.
