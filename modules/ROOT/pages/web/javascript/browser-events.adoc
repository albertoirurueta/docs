...
"register as a capturing handler" -- see <<Event Propagation: Capturing, Target, and Bubbling>>) or an options
object with three recognized properties:
...
Most events bubble; the notable exceptions are `"focus"`, `"blur"`, and `"scroll"`. The `"load"` event bubbles
on document elements but stops at `Document` -- it never reaches `Window` for an individual element's load; the
`Window`'s own `"load"` event fires separately once the whole page has finished loading.
