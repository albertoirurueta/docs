...
variable. Because of this, a project's own Sass file can set the same variable *before* importing Bootstrap,
and Bootstrap's default is skipped entirely -- no patching of Bootstrap's source, no post-processing the
compiled CSS.
...
// 1. Override individual variables before Bootstrap declares its !default values
$primary:    #6f42c1;
$font-family-sans-serif: "Inter", system-ui, sans-serif;
$border-radius: 0.5rem;

// 2. Override a whole map at once
...
// 3. Only then import Bootstrap itself
@use "bootstrap/scss/bootstrap" as *;
----

The ordering is not stylistic -- it is required. `@use "bootstrap/scss/bootstrap"` is where every `!default`
assignment actually executes, so any variable a project wants to change must be set *before* that line, exactly
as covered for any Sass library in xref:web/sass/variables.adoc[Variables]'s `!default` section.
...
Skipping `@use "bootstrap/scss/card"`, `.../modal`, `.../carousel`, and every other unused component partial
...
flowchart LR
    ...
    IMPORT["@import/@use 'bootstrap'<br/>(full or trimmed partials)"]
