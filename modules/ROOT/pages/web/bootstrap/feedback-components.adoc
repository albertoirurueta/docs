...
custom JavaScript; `aria-labelledby` pointing at the header's title `id`, plus `aria-hidden="true"` on the
closed modal itself (which Bootstrap's JavaScript flips to `false` while it is open), are what make the modal
intelligible to a screen reader -- both are part of the standard markup, not optional extras.
...
By default, clicking outside a modal (on its dimmed backdrop) closes it, as does pressing kbd:[Esc]. Setting
...
`data-bs-keyboard="false"` additionally disables the kbd:[Esc] shortcut, though the modal's own explicit close
