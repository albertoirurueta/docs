...
:description: Covers CSS float, flexbox, and grid layout -- how each positions elements relative to one another, clearing floats, flex container/item sizing and alignment, grid tracks/lines/named areas -- plus responsive techniques shared by all three, including media and container queries, responsive images, dynamic viewport units, and scroll-snap.
:keywords: CSS layout, float, clearfix, flow-root, flexbox, flex-direction, justify-content, align-items, flex-grow, flex-shrink, flex-basis, CSS grid, grid-template-columns, grid-template-areas, grid-template-rows, media queries, container queries, responsive images, srcset, dynamic viewport units, scroll-snap
