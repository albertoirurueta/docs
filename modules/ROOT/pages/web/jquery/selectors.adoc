...
you can iterate with `for...of` or spread into an array. There is no native counterpart for the jQuery
extensions (`:eq()`, `:has()` is now shipping in modern browsers, `:contains()`, `:visible`) -- express those
as a standard selector followed by an `Array.prototype.filter`. See
xref:web/javascript/browser-dom-basics.adoc[Web programming basics] and
