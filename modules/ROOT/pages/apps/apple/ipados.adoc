...
xref:apps/apple/app-store-connect-testflight-and-review.adoc#_upcoming_requirements_and_deadlines[Upcoming Requirements and Deadlines]
for the full list.

See https://developer.apple.com/news/?id=k1mtkt1k[Apple Developer News -- App Store submissions now open for the latest OS releases]
and https://developer.apple.com/news/upcoming-requirements/[Apple Developer -- Upcoming Requirements].
...
and link:++https://developer.apple.com/documentation/swiftui/view/sheet(ispresented:ondismiss:content:)++[+Apple Developer Documentation -- sheet(isPresented:onDismiss:content:)+].
...
* **The inspector is guarded, and visionOS gets a sheet instead.** `inspector(isPresented:content:)` exists on iOS,
  iPadOS and macOS but not on visionOS, and this file is compiled for every platform of the app target. So that the
  *Info* button does something everywhere, the visionOS build presents the same `NoteMetadataInspector` in a sheet
  with a *Done* button, driven by the same `showsInspector` state.
...
| Not available | Yes | Yes | Not available | Yes

| `WindowGroup(id:for:)`, `openWindow(id:value:)`
| Yes (16) | Yes (16) | Yes (13) | Not available | Yes

| `+windowScene(_:didUpdateEffectiveGeometry:)+`, `isInteractivelyResizing`
| New in 26 | New in 26 | Not available (Mac Catalyst 26) | Not available | New in 26

| `UISceneClosureConfirmation`
| New in 27 | New in 27 | Not available (Mac Catalyst 27) | Not available | Not available
...
| Yes (13.4) | Yes (13.4) | Not available (Mac Catalyst only) | Not available | Yes

| `onPencilDoubleTap`, `onPencilSqueeze`, `PencilHoverPose`
| Yes (17.5) | Yes (17.5) | Yes (14.5) | Not available | Yes (26.2)

| `UIPencilInteraction` tap and squeeze callbacks
| Yes (17.5) | Yes (17.5) | Not available (Mac Catalyst only) | Not available | Yes (26.2)

| PencilKit `PKCanvasView`
| Yes | Yes | Not available (Mac Catalyst only) | Not available | Yes

| `Table`, `inspector(isPresented:content:)`
| Yes (16 / 17) | Yes (16 / 17) | Yes | Not available | Yes (`Table` only)
|===
...
* link:++https://developer.apple.com/documentation/swiftui/view/sheet(ispresented:ondismiss:content:)++[+Apple Developer Documentation -- sheet(isPresented:onDismiss:content:)+]
