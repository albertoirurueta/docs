...
keyboard shortcut. `NotesTable` is compiled for visionOS too, where tables exist but inspectors don't; the Vision
build shows the metadata in an ornament instead, described on
xref:apps/apple/visionos.adoc[visionOS: Windows, Volumes, Immersive Spaces and RealityKit], so the `.inspector` call
sits behind `+#if !os(visionOS)+` in the real file.
