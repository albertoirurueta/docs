...
A `@Model` subclass always needs an explicit `@available` annotation naming the platforms it supports -- SwiftData's
macro cannot infer one, even when it would just match the app's deployment minimum. *FieldNotes* marks the subclass
`+@available(iOS 26, macOS 26, watchOS 26, visionOS 26, *)+` to match its deployment minimum of 26, as Apple's
sample does.
...
image::apple-swiftdata-app-group.svg[The FieldNotes app process and its widget and share extensions each open ModelContainer.fieldNotes on the same FieldNotes store inside the App Group container group.com.example.fieldnotes. Every process opens it with the same migration plan; only the app mirrors the store to the CloudKit private database iCloud.com.example.fieldnotes, and extensions open it without CloudKit. The Apple Watch app has its own store on the watch and reaches the same data through CloudKit,width=760,role=text-center]
...
because it queues every write in order. xref:apps/apple/camera-media-and-photos.adoc[Camera, Media and Photos] extends the same actor
