...
xref:apps/react-native/storage-and-offline.adoc[Storage and Offline Support].
...
* **`@Attribute(_:)`** changes how one property is stored. `.externalStorage` lets SwiftData keep a large `Data`
  ...
  <<_persistent_history>>). The other options are `.unique`, `.spotlight`, `.allowsCloudEncryption`,
  `.ephemeral` (track changes but don't persist), `.transformable(by:)`, and `.codable`, which is new in 27.
...
let audioOnly = FetchDescriptor<CaptureRecord>(predicate: #Predicate { $0 is AudioCaptureRecord })
----
...
[source,swift]
----
MigrationStage.lightweight(fromVersion: FieldNotesSchemaV1.self, toVersion: FieldNotesSchemaV2.self)
----
...
only in the main app. Extensions open the store with the current schema and no plan, and they are never the first
process to open a store after an update, because the system launches the updated app before it asks its widgets
for new timelines, and the user opens the app to capture notes. An extension that finds an older store fails to
open it, shows its placeholder, and succeeds the next time after the app has run.
...
struct RecordSearchResults: View {
    ...
    init(searchText: String) {
        ...
        _results = Query(filter: predicate,
                         sort: [SortDescriptor(\NoteRecord.isPinned, order: .reverse),
                                SortDescriptor(\NoteRecord.createdAt, order: .reverse)])
    }
...
[source,swift]
----
// Inside SwiftDataNotePersistence (a model actor, shown below)
func pinnedNoteSummary(limit: Int) throws -> (total: Int, newest: [Note]) {
    ...
    var body: some View {
        List {
            ForEach(_notes.sections) { section in
                Section(section.id ?? String(localized: "Unknown place")) {
...
Inside a model actor, the `self[identifier, as: NoteRecord.self]` subscript does the same lookup in the actor's
own context. An identifier of a model that was inserted but not saved yet is temporary, so save before sending one
to another context.
...
the `SampleNotes` modifier from xref:apps/apple/xcode-projects-and-previews.adoc[Xcode Projects, Targets and
Previews] keeps working for every view that only reads `NoteStore`.
...
newer transactions. History grows with every save; once every consumer has processed it, `deleteHistory(_:)` with
a descriptor for older transactions reclaims the space.
...
Lifecycle] describes for `scenePhase`.
...
views query it with `@Query`, edit it through `@Bindable`, and insert and delete through the environment's
...
xref:apps/apple/swiftui-controls-forms-text-and-images.adoc[Controls, Forms, Text and Images] edits with
`TextEditor`:
...
* xref:apps/apple/background-execution.adoc[Background Execution] -- keeping the store fresh with background
  tasks.
* xref:database/couchbase/sdks-and-mobile.adoc[Couchbase SDKs and Mobile] -- Couchbase Lite, a third-party
