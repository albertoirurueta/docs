...
(xref:apps/android/kotlin/collections-and-sequences.adoc[Collections and Sequences]) are built on:
...
xref:apps/android/kotlin/control-flow.adoc[Control Flow]'s `forEach` example, which is itself `inline`):
...
* xref:apps/android/kotlin/control-flow.adoc[Control Flow] -- labeled `return@label` as the alternative to a
  non-local return.
...
* xref:apps/android/kotlin/type-safe-builders-and-dsls.adoc[Type-Safe Builders and DSLs] -- lambdas with an
