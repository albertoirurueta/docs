...
| Transport | Browser-mediated: a hidden `<iframe>` per client, loaded by the browser | Server-to-server: the AS `POST`s directly to each client's backend, no browser involved
...
logout_token=eyJhbGciOiJSUzI1NiJ9..signature
----
...
The client verifies the signature, `iss`, `aud` and `jti` (replay protection) exactly as it would an ID token,
