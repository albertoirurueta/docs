...
* On receiving a back-channel logout token, the BFF looks up the local session(s) matching `sid` / `sub`,
  deletes them from its session store (Redis, a database table, whatever backs it -- see the
  xref:database/redis/use-cases-and-patterns.adoc[Redis Use Cases & Patterns] page for using Redis as a session
  store), and revokes the corresponding access and refresh tokens at the AS.
