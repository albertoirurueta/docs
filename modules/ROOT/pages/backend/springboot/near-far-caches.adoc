...
invalidation, not replaced by it, so a missed message self-heals instead of lasting indefinitely. See
xref:database/redis/streams-and-pub-sub.adoc[Redis: Streams and Pub/Sub] for deeper coverage of the messaging
mechanics, and xref:database/redis/clients-and-client-side-caching.adoc[Redis: Clients and Client-Side
Caching] for Redis's built-in client-side caching, which delivers these invalidations without a hand-rolled
message bus.
