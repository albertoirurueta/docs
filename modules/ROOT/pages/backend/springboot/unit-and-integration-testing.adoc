...
substitutes that behave differently from production. How Testcontainers talks to Docker is explained in
xref:backend/docker/testcontainers-fundamentals.adoc[Testcontainers Fundamentals], and running these tests
against a shared `compose.yaml`, locally and in GitHub Actions, in
...
See xref:database/redis/spring-boot-redis-as-a-cache.adoc[Spring Boot: Redis as a Cache] for a full
`@ServiceConnection` Testcontainers recipe against a Redis module.
...
For the Quarkus equivalent -- `@QuarkusTest`, `@QuarkusComponentTest`, and `@QuarkusIntegrationTest` in place of
`@SpringBootTest` and its test slices -- see xref:backend/quarkus/testing.adoc[Testing Quarkus Applications].
