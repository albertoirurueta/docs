= Spring Data Neo4j
:description: Mapping domain objects to graph nodes and relationships with Spring Data Neo4j, node inheritance via multiple labels, querying them through repositories, derived and custom Cypher queries, projections, auditing, @Transactional transactions, @Version optimistic locking, and low-level Cypher access with Neo4jClient.
:keywords: Spring Data Neo4j, SDN, Neo4j, @Node, @Relationship, @Id, @GeneratedValue, node inheritance, multiple labels, Neo4jRepository, derived query methods, @Query, Cypher, projections, auditing, @CreatedDate, @Version, optimistic locking, Neo4jClient, fetchAs, @Transactional, Neo4jTransactionManager, transactions
...
See https://neo4j.com/docs/cypher-manual/current/values-and-types/spatial/[Neo4j Cypher spatial values and
functions] and https://docs.spring.io/spring-data/neo4j/reference/[the Spring Data Neo4j reference guide].
...
Repository methods are transactional on their own, so a boundary is only needed when several operations must
succeed or fail together, exactly as in the other Spring Data modules.
...
See xref:backend/springboot/transaction-isolation-and-locking.adoc[Transaction Isolation & Locking] for how this
...
* xref:database/neo4j/property-graph-model.adoc[The Property Graph Model] -- the nodes, relationships, labels and
  properties that `@Node`, `@Relationship` and `@RelationshipProperties` map Java types onto.
* xref:database/neo4j/cypher-fundamentals.adoc[Cypher Fundamentals] -- the query language behind every derived
  finder and `@Query` annotation on a `Neo4jRepository`.
* xref:database/neo4j/indexes-and-constraints.adoc[Indexes & Constraints] -- how to back `@Id`/unique fields and
  frequently filtered properties with the range, uniqueness and node-key constraints Spring Data itself does not
  create for you.
* xref:database/neo4j/transactions-and-drivers.adoc[Transactions & Drivers] -- the Bolt driver transaction
  functions that `Neo4jTransactionManager` and `Neo4jClient` are built on underneath `@Transactional`.
* xref:database/neo4j/graph-data-science-fundamentals.adoc[Graph Data Science Fundamentals] +
  xref:database/neo4j/pathfinding-and-centrality-algorithms.adoc[Pathfinding & Centrality Algorithms] -- running
  GDS algorithms over data mapped with this page's entities, beyond what repository queries can express.
* xref:database/neo4j/vector-search-and-genai.adoc[Vector Search & GenAI] -- vector indexes and GraphRAG
  patterns for building retrieval-augmented search on top of a Spring Data Neo4j domain model.
