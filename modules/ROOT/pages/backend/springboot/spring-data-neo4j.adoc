...
public class CastingService {
    ...
    @Transactional
    public void castActor(String movieTitle, String actorName, String role) {
        Movie movie = movies.findByTitle(movieTitle).orElseThrow();
        Person actor = people.findByName(actorName).orElseThrow();
        movie.getCast().add(new Role(actor, role));
        movies.save(movie);          // both the node and the new relationship commit together
    }
}
----
...
A custom `@Transactional` isolation level is *not* supported: `Neo4jTransactionManager` does not accept a
non-default `Isolation` value, because Neo4j does not expose the ANSI levels. Neo4j transactions run at
