...
xref:backend/springboot/api-first-messaging.adoc[API-First: Messaging]. See
xref:backend/messaging/kafka-architecture.adoc[Apache Kafka Architecture] for the broker concepts (topics,
partitions, replication, KRaft) underlying everything here, and
xref:backend/messaging/spring-boot-kafka.adoc[Spring Boot: Apache Kafka] for the recipes not covered here
(topic provisioning, non-blocking `@RetryableTopic` retries, share consumers, Kafka Streams, and more).
...
`DeadLetterPublishingRecoverer`. See
xref:backend/messaging/messaging-patterns-in-practice.adoc[Messaging Patterns in Practice] for how this retry
and dead-lettering approach fits into the broader retry/DLQ pattern shared across Kafka and RabbitMQ:
...
For the Quarkus equivalent of this whole page -- SmallRye Reactive Messaging's `@Incoming`/`@Outgoing` channels
over the same Kafka broker -- see xref:backend/quarkus/messaging.adoc[Messaging with SmallRye Reactive
Messaging].
