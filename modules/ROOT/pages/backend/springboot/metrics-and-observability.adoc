...
This page only covers the application side of that pipeline -- exposing metrics and wiring the scrape
config/data source shown above. For the full Prometheus and Grafana reference beyond that -- PromQL,
recording and alerting rules, Alertmanager, service discovery, storage and remote write, and Grafana
dashboarding and alerting in depth -- see xref:database/prometheus/index.adoc[Prometheus & Monitoring].
...
For the Quarkus equivalent of this whole page -- Micrometer, `/q/metrics`, and OpenTelemetry tracing/logging
built the same way on top of the same standards -- see
xref:backend/quarkus/observability.adoc[Observability: Health, Logging, Metrics, Tracing].
