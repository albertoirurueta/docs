...
xref:backend/springboot/spring-data-overview.adoc#_keyset_scrolling_with_windowt[Keyset scrolling with
Window<T>]):
...
one. See https://postgis.net/docs/reference.html[the PostGIS spatial function reference].
...
The same annotation also carries an `isolation` attribute --
`@Transactional(isolation = Isolation.REPEATABLE_READ)` -- which overrides the database session's default level
...
xref:backend/springboot/transaction-isolation-and-locking.adoc[Transaction Isolation & Locking] for the levels
and their cost, and xref:database/sql/transactions.adoc[SQL Reference: Transaction Control] for the underlying
ANSI model.
...
over R2DBC or reactive MongoDB, where the transaction rides the Reactor `Context` rather than the thread --
see xref:backend/springboot/reactive-programming.adoc[Reactive transactions and thread affinity].
...
xref:backend/springboot/transaction-isolation-and-locking.adoc[Transaction Isolation & Locking] for the other
