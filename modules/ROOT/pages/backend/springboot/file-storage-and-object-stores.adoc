...
driver], which recent GKE versions enable by default as a managed add-on. Like EFS, it is `ReadWriteMany` --
built to be read and written concurrently from many pods at once.
