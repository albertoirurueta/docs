...
parameter types such as `Point`, `Circle`, `Box`, and `Polygon`. A `Near` query can additionally take a
...
`Metrics.KILOMETERS` / `Metrics.MILES` make `Distance` unit-aware, so callers never convert units manually. See
