...
xref:database/mongodb/crud-query.adoc#_avoiding_large_skips[MongoDB's own `skip()` alternative], instead of the
...
database level. See the xref:database/mongodb/special-indexes-and-search.adoc[Special Indexes & Search] page for
...
See https://docs.spring.io/spring-data/mongodb/reference/mongodb/geo-json.html[GeoJSON support in Spring Data
MongoDB].
...
`NearQuery` API reference] and https://www.mongodb.com/docs/manual/geospatial-queries/[MongoDB's geospatial
queries overview].
...
xref:database/mongodb/transactions.adoc[MongoDB Reference: Transactions] for those settings, and
xref:backend/springboot/transaction-isolation-and-locking.adoc[Transaction Isolation & Locking] for how this
