...
`@Transactional`'s `isolation` attribute is *not* supported here: `MongoTransactionManager` rejects any
non-default `Isolation` value with `InvalidIsolationLevelException`, because MongoDB does not expose the ANSI
