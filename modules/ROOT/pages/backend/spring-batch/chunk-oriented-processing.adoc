...
Both forms are supported. The `StepBuilder.chunk(...)` idiom above is the one most existing code and most
examples use, and it remains current; the `ChunkOrientedStepBuilder` is the entry point when the newer
producer/consumer options are wanted.
