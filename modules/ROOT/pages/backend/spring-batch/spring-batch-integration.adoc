...
`ChunkMessageChannelItemWriter` that sends the chunk to a request channel and correlates the reply. The
`Amqp.outboundAdapter`/`Amqp.inboundAdapter` wiring below rides on the same `spring-boot-starter-amqp` and
`ConnectionFactory` covered in xref:backend/messaging/spring-boot-rabbitmq.adoc[Spring Boot: RabbitMQ].
...
xref:backend/spring-batch/getting-started.adoc[Getting started] lists as removed -- the `factory.get("name")`
