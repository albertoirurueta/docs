...
guards -- a full treatment is on xref:backend/java/pattern-matching.adoc[Pattern Matching].
...
* xref:backend/java/pattern-matching.adoc[Pattern Matching] -- type patterns, record patterns, and
  `when` guards in `switch`.
* xref:backend/java/operators-and-expressions.adoc[Operators and Expressions] -- the ternary
  conditional operator and boolean short-circuiting.
* xref:backend/java/arrays.adoc[Arrays] -- the enhanced `for` over arrays.
* xref:backend/java/enums.adoc[Enums] -- `switch` over enum constants and exhaustiveness.
