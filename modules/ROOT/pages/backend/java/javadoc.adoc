...
xref:backend/java/build-and-tooling.adoc#generating-api-docs[Build & Tooling].
...
| Marks the member deprecated in the generated documentation and explains what to use instead. Pair it with the
  `@Deprecated` annotation (see xref:backend/java/annotations-and-reflection.adoc[Annotations and Reflection]):
...
xref:backend/java/build-and-tooling.adoc#generating-api-docs[Build & Tooling] for the CLI option itself.
...
See xref:backend/java/build-and-tooling.adoc#generating-api-docs[Generating API Docs with javadoc] for the
...
* Run with `-Xdoclint` (or the Maven/Gradle equivalents from
  xref:backend/java/build-and-tooling.adoc[Build & Tooling]) in CI, so a broken doc comment fails the build
  instead of shipping.
