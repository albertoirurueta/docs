...
xref:backend/java/enums.adoc[Enums]. Interface constants remain common for genuinely primitive
protocol values.
...
* xref:backend/java/inheritance-and-polymorphism.adoc[Inheritance and Polymorphism] -- class inheritance,
  which these rules interact with (class wins over interface).
* xref:backend/java/lambdas-and-method-references.adoc[Lambdas and Method References] -- implementing
  functional interfaces without a named class.
* xref:backend/java/records-and-sealed-classes.adoc[Records and Sealed Classes] -- records implementing
  interfaces, and `sealed interface`.
* xref:backend/java/generics.adoc[Generics] -- type parameters on interfaces such as `Comparable<T>` and
  `Comparator<T>`.
