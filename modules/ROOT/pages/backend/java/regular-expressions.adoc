...
* xref:backend/java/strings-and-text.adoc[Strings and Text] -- `String.split`, `replace` vs.
  `replaceAll`, and text blocks for readable multi-line patterns.
* xref:backend/java/dates-and-times.adoc[Dates and Times] -- validating loose date input before
  handing it to a `DateTimeFormatter`.
* xref:backend/java/exceptions.adoc[Exceptions] -- catching `PatternSyntaxException` from patterns
  built at run time.
* xref:backend/java/streams-and-collectors.adoc[Streams and Collectors] -- `Matcher.results()` and
