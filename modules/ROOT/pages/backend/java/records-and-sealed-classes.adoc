...
* xref:backend/java/pattern-matching.adoc[Pattern Matching] -- type patterns, record deconstruction,
  and guarded `switch` labels in depth.
* xref:backend/java/classes-and-objects.adoc[Classes and Objects] -- the constructor, field, and
  `equals`/`hashCode` mechanics a record automates.
* xref:backend/java/interfaces.adoc[Interfaces] -- `sealed interface` and the interfaces a record can
  implement.
* xref:backend/java/enums.adoc[Enums] -- the other closed, exhaustively switchable type.
