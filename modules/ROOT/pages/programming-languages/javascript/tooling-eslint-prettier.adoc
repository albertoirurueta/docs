...
:description: A comparison of ESLint and Prettier for JavaScript code quality, covering ESLint's flat config and rule severities with a worked example of catching a real bug, Prettier's formatting-only role, and how eslint-config-prettier composes the two tools without conflicting fixes.
:keywords: JavaScript, ESLint, Prettier, flat config, eslint.config.js, static analysis, linting, code formatting, eslint-config-prettier, no-unused-vars, auto-fix, editor integration, pre-commit hook
:page-aliases: web/javascript/tooling-eslint-prettier.adoc
...
are composed so they don't fight each other. See also xref:programming-languages/javascript/tooling-babel.adoc[Babel & Build-Time
Transforms] for how these tools fit alongside a transpilation step, and
xref:programming-languages/javascript/tooling-jest.adoc[Testing with Jest] for the third leg of a typical JavaScript quality
toolchain.
