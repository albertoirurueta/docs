...
:description: A guide to JavaScript's RegExp class covering literal and constructor syntax, character classes, quantifiers, capturing and named groups, anchors and lookaround, flags, and the String/RegExp methods that use regular expressions, including the lastIndex statefulness pitfalls of the g and y flags.
:keywords: JavaScript, RegExp, regular expressions, regex, character classes, quantifiers, capture groups, named capture groups, backreferences, lookahead, lookbehind, anchors, flags, global flag, sticky flag, lastIndex, test, exec, match, matchAll, replace, replaceAll, split, search
:page-aliases: web/javascript/stdlib-regexp.adoc
...
* xref:programming-languages/javascript/stdlib-utilities.adoc[Standard Library: Console, URL & Timers] for other small
