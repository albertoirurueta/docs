...
:page-aliases: web/javascript/browser-events.adoc
...
xref:programming-languages/javascript/browser-dom-basics.adoc[DOM Basics].
...
* `currentTarget` vs. `target` -- inside a delegated handler, `event.currentTarget` is always the ancestor the
  ...
  `event.target.closest(selector)` (see xref:programming-languages/javascript/browser-dom-basics.adoc[DOM Basics]) is the standard
...
* xref:programming-languages/javascript/browser-dom-basics.adoc[DOM Basics] -- selecting the elements you attach listeners to
  (`querySelector()`, `querySelectorAll()`, `closest()`) and traversing the document tree.
* xref:programming-languages/javascript/async-javascript.adoc[Asynchronous JavaScript] -- how event-driven code relates to
  Promises, the microtask/macrotask queues, and `async`/`await`.
* xref:programming-languages/javascript/functions-expressions-operators.adoc[Functions, Expressions & Operators] -- arrow function
