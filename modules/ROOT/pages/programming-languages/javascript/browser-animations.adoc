...
:description: Covers driving animation from JavaScript by toggling CSS classes and custom properties, building requestAnimationFrame render loops, using the Web Animations API's element.animate(), and animating inline SVG shapes.
:keywords: JavaScript animation, requestAnimationFrame, Web Animations API, element.animate, CSS transitions, animating SVG, cancelAnimationFrame, prefers-reduced-motion
:page-aliases: web/javascript/browser-animations.adoc
...
xref:programming-languages/javascript/browser-css.adoc[Accessing CSS from JavaScript] (see "Reacting to Animation and Transition
...
* A `requestAnimationFrame` loop is also the standard way to drive per-frame drawing on `<canvas>` -- see
  xref:programming-languages/javascript/browser-canvas-webgl.adoc[Canvas, WebGL & Three.js] for that render-loop pattern.
