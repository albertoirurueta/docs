= Storage
:description: A comparison of client-side storage in the browser -- cookies and their attributes, the localStorage/sessionStorage Web Storage API and its storage event, and the asynchronous, transactional IndexedDB database.
:keywords: cookies, document.cookie, max-age, localStorage, sessionStorage, Web Storage, storage event, IndexedDB, object store, transaction, keyPath, structured clone, client-side storage, origin scoping
:page-aliases: web/javascript/browser-storage.adoc
...
xref:programming-languages/javascript/stdlib-dates-errors-json.adoc[JSON]) is the usual choice for structured data:
...
xref:programming-languages/javascript/browser-events.adoc[Events] for `addEventListener()` details):
...
so on -- see xref:programming-languages/javascript/stdlib-collections.adoc[Maps, Sets & WeakRefs]) in named *object stores*, and it
...
xref:programming-languages/javascript/async-javascript.adoc[Asynchronous JavaScript] -- so real code typically wraps each request
