...
:page-aliases: web/javascript/browser-location-navigation-history.adoc
...
xref:programming-languages/javascript/browser-networking.adoc[Networking]): its properties expose the individual parts of the
...
[IMPORTANT]
====
The Geolocation API has no dedicated coverage in this section's reference material -- it's mentioned only as a
further-reading pointer. Everything in this section is drawn from general/official knowledge (MDN).
====
