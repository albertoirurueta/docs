...
:description: A full JavaScript operator reference -- arithmetic, comparison, == vs === equality with a side-by-side comparison table, logical, nullish coalescing, optional chaining, and bitwise operators -- plus function fundamentals covering declarations, expressions, arrow functions, this binding, and IIFEs.
:keywords: JavaScript, operators, arithmetic operators, comparison operators, equality operators, loose equality, strict equality, logical operators, nullish coalescing, optional chaining, bitwise operators, typeof, instanceof, ternary operator, function declarations, function expressions, arrow functions, this binding, IIFE
:page-aliases: web/javascript/functions-expressions-operators.adoc
...
value, and **statements** (see xref:programming-languages/javascript/statements.adoc[Statements]), which do things -- branching,
...
NOTE: Automatic semicolon insertion (see xref:programming-languages/javascript/lexical-structure.adoc[Lexical Structure]) means
...
| `true`
| `false`
| `"1"` converts to the number `1` for `==`; `===` never converts, and a number is never the same type as a
  string.

| `0` / `false`
| `true`
| `false`
| `false` converts to `0` for `==`; different types fail `===` immediately.

| `null` / `undefined`
| `true`
| `false`
| `==` special-cases this exact pair as equal to each other (and to nothing else); `===` treats them as distinct
  types.

| `NaN` / `NaN`
| `false`
| `false`
| `NaN` is never equal to anything, including itself, under either operator -- use `Number.isNaN()` to test for
  it.

| `0` / `-0`
| `true`
| `true`
| Zero and negative zero are considered equal by both operators (`Object.is(0, -0)` is the rare case that tells
  them apart).

| `""` / `false`
| `true`
| `false`
| `==` converts both operands to numbers (`""` → `0`, `false` → `0`); `===` sees a string and a boolean, two
  different types.

| `[]` / `false`
| `true`
| `false`
| `==` converts the array to its primitive form (`""`, via `toString()`) and then to a number (`0`), matching
  `false`'s converted `0`; `===` sees an object and a boolean.

| `{}` / `{}`
| `false`
| `false`
| Two distinct object literals are two distinct references -- neither operator considers them equal no matter how
  similar their contents are.
|===

NOTE: `==`'s conversions are about *type*, not *truthiness* -- don't confuse this table with the separate
truthy/falsy rules a bare `if (value)` test applies. See
xref:programming-languages/javascript/types.adoc#_boolean_values[Types, Values & Conversions: Boolean Values] for the truthy/falsy
...
xref:programming-languages/javascript/classes.adoc[Classes]) so subclass instances still test positive for a superclass:
...
used as constructors with `new` (see xref:programming-languages/javascript/classes.adoc[Classes] for constructors proper). As a
...
* **Plain function call** (`f()`) -- `this` is the global object in non-strict mode, or `undefined` in strict
  mode (see xref:programming-languages/javascript/lexical-structure.adoc[Lexical Structure]).
...
global namespace. ES modules (see xref:programming-languages/javascript/modules.adoc[Modules]) give every module file its own
top-level scope automatically, which has superseded this trick for that purpose -- see
xref:programming-languages/javascript/function-parameters-namespaces.adoc[Function Parameters & Namespaces] for the related
