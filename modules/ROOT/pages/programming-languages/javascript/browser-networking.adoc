= Networking
:description: A guide to browser networking APIs -- fetch() requests and Response handling, Server-Sent Events via EventSource, bidirectional WebSocket connections, and peer-to-peer WebRTC with signaling and ICE/STUN/TURN.
:keywords: fetch, Fetch API, Response, Headers, Request, AbortController, streaming, ReadableStream, CORS, FormData, Server-Sent Events, EventSource, WebSocket, WebRTC, RTCPeerConnection, ICE, STUN, TURN, signaling, MediaStream, RTCDataChannel
:page-aliases: web/javascript/browser-networking.adoc
...
See xref:programming-languages/javascript/async-javascript.adoc[Asynchronous JavaScript] for a full worked example of the same
...
xref:programming-languages/javascript/async-javascript.adoc[Asynchronous JavaScript] page); there is no reason to reach for `XHR`
in new code.
...
xref:programming-languages/javascript/stdlib-utilities.adoc[Utilities]) rather than assembled by hand.
...
| `response.arrayBuffer()` | An `ArrayBuffer`, for building a typed array or `DataView` over binary response
  data (see xref:programming-languages/javascript/arrays.adoc[Arrays & Typed Arrays]).
...
xref:programming-languages/javascript/iterators-generators.adoc[iterator protocol]:
...
with a matching `Access-Control-Allow-Origin` header -- otherwise `fetch()` rejects. See xref:web/cors.adoc[] for
