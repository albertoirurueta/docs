...
:description: Covers JavaScript's primitive types -- numbers (including BigInt and floating-point precision), strings and template literals, booleans and truthy/falsy values, null and undefined, and symbols -- along with implicit coercion, the == versus === comparison, and explicit conversion functions.
:keywords: JavaScript types, primitives, Number, BigInt, floating-point precision, safe integers, strings, template literals, tagged templates, UTF-16, Boolean, truthy, falsy, null, undefined, Symbol, type conversion, coercion, ==, ===, parseInt, parseFloat
:page-aliases: web/javascript/types.adoc
...
including xref:programming-languages/javascript/arrays.adoc[Arrays & Typed Arrays], covered on their own page rather than here).
...
xref:programming-languages/javascript/iterators-generators.adoc[Iterators & Generators]). `Symbol.for(key)` offers an alternative,
...
* xref:programming-languages/javascript/arrays.adoc[Arrays & Typed Arrays] -- the `Array` object type, and the binary-data typed
  array constructors built on the same numeric formats described above.
* xref:programming-languages/javascript/objects-destructuring.adoc[Objects & Destructuring] -- how object property values (as
  opposed to the primitive types on this page) are read, written, and destructured.
* xref:programming-languages/javascript/iterators-generators.adoc[Iterators & Generators] -- the well-known symbols
  (`Symbol.iterator` and others) that make an object work with `for...of` and the spread operator.
* xref:programming-languages/javascript/lexical-structure.adoc[Lexical Structure] -- how numeric and string literals are tokenized
  at the source level.
