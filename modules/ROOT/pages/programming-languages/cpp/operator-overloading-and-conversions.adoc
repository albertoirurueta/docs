...
:description: Member vs. non-member operators, operator<=> and defaulted comparisons, operator[] (multidimensional in {cpp}23), operator(), conversion operators, explicit, user-defined literals, stream operators.
:keywords: {cpp}, operator overloading, operator<=>, operator[], operator(), conversion operator, explicit, stream operators
...
== `operator[]` (Multidimensional in {cpp}23)

{cpp}23 allows `operator[]` to take *multiple* arguments (previously limited to exactly one), so a matrix-like
...
A conversion operator lets a user-defined type convert *to* another type; `explicit` on it ({cpp}11) blocks
