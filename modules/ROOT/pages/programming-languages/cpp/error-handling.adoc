...
:keywords: C++, exceptions, exception hierarchy, try catch, exception safety, exception_ptr, error_code, system_error, std::expected, assert, source_location, stacktrace, std::terminate
...
public:
    void addItem(int item) {
        auto backup = items_;          // copy first (strong guarantee): if push_back throws, backup is
        items_.push_back(item);          // unaffected and items_ is simply reassigned back on failure
    }
private:
...
C++20's `std::source_location` captures the calling file/line/function *without* the `__FILE__`/`__LINE__`
