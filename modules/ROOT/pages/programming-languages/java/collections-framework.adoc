...
:page-aliases: backend/java/collections-framework.adoc
...
factories (`comparing`, `thenComparing`, `reversed`) is covered on xref:programming-languages/java/interfaces.adoc[Interfaces].
...
* xref:programming-languages/java/streams-and-collectors.adoc[Streams and Collectors] -- processing the contents of a
  collection with a lazy pipeline instead of an explicit loop.
* xref:programming-languages/java/generics.adoc[Generics] -- the type parameters (`List<E>`, `Map<K, V>`, bounded
  wildcards) that every collection interface is built on.
* xref:programming-languages/java/interfaces.adoc[Interfaces] -- `Comparable` vs. `Comparator` and the factory methods
  used to sort `TreeSet`, `TreeMap`, and `PriorityQueue`.
* xref:programming-languages/java/inheritance-and-polymorphism.adoc[Inheritance and Polymorphism] -- the full
