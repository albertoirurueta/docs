...
:page-aliases: backend/java/streams-and-collectors.adoc
...
`findFirst`, `findAny`, and the no-identity `reduce` return xref:programming-languages/java/optional.adoc[`Optional`]
...
* xref:programming-languages/java/collections-framework.adoc[The Collections Framework] -- the `Collection.stream()`
  sources and the `List`/`Set`/`Map` targets that collectors build.
* xref:programming-languages/java/lambdas-and-method-references.adoc[Lambdas and Method References] -- the syntax every
...
* xref:programming-languages/java/optional.adoc[Optional] -- the return type of `findFirst`, `findAny`, `min`, `max`,
