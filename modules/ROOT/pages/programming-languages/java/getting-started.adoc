= Getting Started
:description: What Java is and how the JDK differs from the JRE, installing and verifying a JDK, compiling with javac and running with java, the single-file source launcher and compact source files, the JShell REPL, IDEs, the six-month release cadence with LTS and preview features, and the path from source to native execution -- the class loader, bytecode verifier, interpreter and JIT compiler.
:keywords: Java, JDK, JRE, OpenJDK, Adoptium, Temurin, javac, java launcher, bytecode, JVM, write once run anywhere, single-file source launcher, compact source file, instance main, JShell, REPL, IDE, IntelliJ IDEA, Eclipse, VS Code, Apache NetBeans, LTS, release cadence, preview features, enable-preview, Runtime.Version, class loader, bytecode verifier, interpreter, JIT compiler
:page-aliases: backend/java/getting-started.adoc
...
choices -- https://www.jetbrains.com/idea/[IntelliJ IDEA], https://eclipseide.org/[Eclipse IDE],
https://code.visualstudio.com/[Visual Studio Code] with the
https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-pack["Extension Pack for Java"], and
https://netbeans.apache.org/[Apache NetBeans] -- are all tool-agnostic here: nothing in this section depends on
...
* xref:programming-languages/java/lexical-structure-and-style.adoc[Lexical Structure and Style] -- how a source file
  is organised once you move past a single `Hello`.
* xref:programming-languages/java/primitive-types-and-variables.adoc[Primitive Types and Variables] -- the values your
  first programs manipulate.
* xref:programming-languages/java/build-and-tooling.adoc[Build and Tooling] -- Maven and Gradle for anything larger
  than a single file.
* xref:programming-languages/java/packages-and-modules.adoc[Packages and Modules] -- organising many classes and
