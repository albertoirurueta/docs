...
:page-aliases: backend/java/concurrency-basics.adoc
...
* xref:programming-languages/java/high-level-concurrency.adoc[High-Level Concurrency] -- executors,
  `CompletableFuture`, concurrent collections, and `java.util.concurrent.atomic` instead of raw
  `synchronized`.
* xref:programming-languages/java/virtual-threads.adoc[Virtual Threads] -- cheap threads for blocking code, and what
  still pins a carrier thread.
* xref:programming-languages/java/exceptions.adoc[Exceptions] -- handling `InterruptedException` and why swallowing
  it is a bug.
* xref:programming-languages/java/collections-framework.adoc[Collections Framework] -- which collections are
