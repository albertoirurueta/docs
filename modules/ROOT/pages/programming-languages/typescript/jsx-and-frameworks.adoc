...
:page-aliases: web/typescript/jsx-and-frameworks.adoc
...
compiler option (see xref:programming-languages/typescript/tsconfig-and-compiler-options.adoc[]) chooses what `<div/>`
compiles to:
...
* `react-native` -- like `preserve`, but emits a `.js` file. See
  xref:apps/react-native/typescript.adoc[TypeScript in React Native] for how React Native projects configure
  TypeScript, including the Strict TypeScript API.
...
xref:programming-languages/typescript/generics.adoc[]). Keep annotations minimal and let inference do the rest.
...
* xref:programming-languages/typescript/tsconfig-and-compiler-options.adoc[] -- the `jsx` and `jsxImportSource` options in context.
* xref:programming-languages/typescript/generics.adoc[] -- a generic component is just a generic function.
