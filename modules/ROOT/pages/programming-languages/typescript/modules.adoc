...
:page-aliases: web/typescript/modules.adoc
...
For the runtime semantics of ES modules themselves, see xref:programming-languages/javascript/modules.adoc[].
...
`isolatedModules` tells TypeScript to check that each file can be transpiled on its own (by Babel, esbuild, swc). It bans `const enum` used across files, plain re-exports of types without the `type` keyword, and other whole-program tricks. Turn it on whenever a bundler -- not `tsc` -- produces your JavaScript. See xref:programming-languages/typescript/tsconfig-and-compiler-options.adoc[].
...
Module *augmentation* reopens a real module to add members -- see xref:programming-languages/typescript/declaration-files.adoc[].
...
xref:programming-languages/typescript/project-references-and-build.adoc[] covers how resolution interacts with composite builds and `tsc --build`.
