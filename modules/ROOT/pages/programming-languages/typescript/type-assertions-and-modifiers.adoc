...
:page-aliases: web/typescript/type-assertions-and-modifiers.adoc

include::partial$typescript-disclaimer.adoc[]

Assertions and modifiers tell the compiler something it cannot infer on its own, or ask it to check a value without changing the type it infers. They sit on top of the inference rules in xref:programming-languages/typescript/the-type-system.adoc[] and the control-flow narrowing in xref:programming-languages/typescript/unions-and-narrowing.adoc[]; reach for them only when inference genuinely falls short.
...
This is the idiomatic way to derive a union type from a runtime list, and to stop an object literal from widening when it is passed to a stricter parameter (see xref:programming-languages/typescript/everyday-types.adoc[]).
