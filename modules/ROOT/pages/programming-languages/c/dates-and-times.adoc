...
* xref:programming-languages/c/threads.adoc[Threads] -- `thrd_sleep` and timed waits, which take a
  `struct timespec`.
* xref:programming-languages/cpp/dates-times-and-chrono.adoc[{cpp}: Dates, Times, and Chrono] -- `<chrono>`
  gives these same facilities type-safe durations and calendar types.
