...
* **`safe`.** A modifier marking a declaration as *not* requiring an unsafe context, so it stays callable from
  safe code; it introduces no safe context of its own and has no block or expression form. The proposal
  requires it on `extern` members and on explicit-layout fields that overlap references.
...
* https://learn.microsoft.com/dotnet/csharp/whats-new/csharp-15[Microsoft Learn -- What's new in C# 15
  (preview)].
* https://github.com/dotnet/csharplang/blob/main/proposals/unsafe-evolution.md[dotnet/csharplang -- Memory
