...
- [x] Task 1. Create `modules/ROOT/partials/c-disclaimer.adoc`
  - [x] Task 1.1. `[IMPORTANT]` admonition, 2–3 sentences, matching the Kotlin/short-template shape: (a) one
...
- [x] Task 2. `getting-started.adoc` — what C is (K&R origins, standardized by ISO/IEC JTC1/SC22 WG14), the
  ...
  - [x] Task 2.1. 📊 SVG `c-compilation-pipeline.svg`: preprocess → compile → assemble → link → run.
- [x] Task 3. `program-structure.adoc` — grammar at a glance, declarations vs. definitions, statements,
      translation units and headers, translation phases, `main` and its signatures, exit status.

#### Language fundamentals

- [x] Task 4. `lexical-structure-and-style.adoc` — character sets, comments, identifiers and reserved names,
      keywords, punctuators, literals overview, formatting/naming conventions, attribute syntax (`[[...]]`).
- [x] Task 5. `basic-types-and-values.adoc` — the abstract state machine (values, types, representations), `bool`,
      ...
      `<limits.h>`/`<float.h>`.
  - [x] Task 5.1. 📊 SVG `c-integer-promotion-ladder.svg`: the integer conversion/promotion ladder.
- [x] Task 6. `constants-enumerations-and-initialization.adoc` — `const` objects, `enum` (including C23 fixed
      underlying types), macros vs. `constexpr`, compound literals, initializers (scalar, aggregate, designated,
      zero/empty `{}`), `static_assert`.
- [x] Task 7. `operators-and-expressions.adoc` — arithmetic, division/remainder, assignment and compound
...
- [x] Task 8. `control-flow.adoc` — `if`/`else`, `switch` with fall-through, `for` (with declaration), `while`,
      `do`-`while`, `break`/`continue`, `goto` and labels, `return`.
  - [x] Task 8.1. 📊 mermaid flowchart of the loop forms (`for`/`while`/`do-while`).
- [x] Task 9. `functions.adoc` — prototypes vs. definitions, pass-by-value, array parameters, `main` is special,
      recursion, `static` (internal linkage) and `inline` functions, `[[noreturn]]`, variadic functions with
      `<stdarg.h>`.
  - [x] Task 9.1. 📊 SVG `c-recursive-call-stack.svg`: call stack frames for a recursive call.
- [x] Task 10. `arrays-and-strings.adoc` — array declaration and length, `sizeof` vs. element count,
...
- [x] Task 11. `structures-unions-and-type-aliases.adoc` — `struct` declaration and designated initialization,
      nested structures, `typedef`, `union` and tagged unions, bit-fields, flexible array members, alignment and
      padding, `offsetof`.
  - [x] Task 11.1. 📊 SVG `c-struct-layout-padding.svg`: struct layout with padding and alignment.

#### Pointers and memory

- [x] Task 12. `pointers.adoc` — address-of and dereference, pointer arithmetic and differences (`ptrdiff_t`),
  ...
  - [x] Task 12.1. 📊 SVG `c-pointer-arithmetic.svg`: pointer, pointee and pointer arithmetic over an array.
- [x] Task 13. `memory-model-and-alignment.adoc` — the uniform memory model, object representation as
...
- [x] Task 14. `dynamic-memory-allocation.adoc` — `malloc`/`calloc`/`realloc`/`free`/`aligned_alloc`/
  ...
  - [x] Task 14.1. 📊 mermaid diagram: lifecycle of a heap object (allocate → use → free).
- [x] Task 15. `storage-duration-scope-and-linkage.adoc` — the four storage durations (static, thread, automatic,
  ...
  - [x] Task 15.1. 📊 mermaid/SVG `c-storage-duration-decision.svg`: storage-duration decision tree.

#### Preprocessor and generic programming

- [x] Task 16. `preprocessor-and-macros.adoc` — object-like and function-like macros, `#include` and include
...
- [x] Task 17. `type-generic-programming.adoc` — inherent generic features (operators, promotions, `void *`,
...
- [x] Task 18. `standard-library-overview.adoc` — header catalogue table (every C23 header, with what it
...
- [x] Task 19. `numbers-and-math.adoc` — `<stdlib.h>` integer functions, `<inttypes.h>` format macros, checked
...
- [x] Task 20. `input-output-and-files.adoc` — streams and `FILE`, `stdin`/`stdout`/`stderr`, unformatted I/O
  ...
  - [x] Task 20.1. 📊 mermaid diagram: stream/buffer flow.
- [x] Task 21. `strings-and-text-processing.adoc` — `<string.h>` (`strlen`, `strcpy`/`strncpy`, `strcat`,
...
- [x] Task 22. `dates-and-times.adoc` — `time_t`, `struct timespec`/`timespec_get`, `clock` and
      `CLOCKS_PER_SEC`, `struct tm` (`gmtime`/`localtime`/`mktime`/`timegm`), `strftime`, `difftime`, measuring
      elapsed time.

#### Robustness and performance

- [x] Task 23. `error-handling-and-program-failure.adoc` — the catalogue of wrongdoings (arithmetic, conversion,
...
- [x] Task 24. `advanced-control-flow.adoc` — sequencing recap, short jumps (`goto` for cleanup), function calls
      as control, long jumps (`setjmp`/`longjmp` and `volatile`), signal handlers (`signal`, `raise`,
      `sig_atomic_t`, async-signal safety).
  - [x] Task 24.1. 📊 mermaid diagram: `setjmp`/`longjmp` unwinding.
- [x] Task 25. `performance.adoc` — measure first, `inline` functions and their linkage rules, `restrict`,
...
- [x] Task 26. `threads.adoc` — `<threads.h>`: `thrd_create`/`thrd_join`, return values, simple inter-thread
  ...
  - [x] Task 26.1. 📊 mermaid sequence diagram: producer/consumer with a condition variable.
- [x] Task 27. `atomics-and-memory-consistency.adoc` — data races and the memory model, `_Atomic` types and
  ...
  - [x] Task 27.1. 📊 SVG `c-happens-before.svg`: happens-before edges between two threads.

#### Tooling and standards

- [x] Task 28. `build-and-tooling.adoc` — GCC/Clang/MSVC invocation and warning flags, `-std=` and feature-test
      ...
      Doxygen.
- [x] Task 29. `testing.adoc` — unit testing in C: `assert`-based self-checks, a minimal test harness, popular
...
- [x] Task 30. `c-standards-and-c23.adoc` — the editions timeline (K&R → C23), what C23 added (`bool`/`true`/
  ...
  - [x] Task 30.1. 📊 mermaid timeline: K&R → C89/90 → C95 → C99 → C11 → C17 → C23.
...
- [x] Task 31. C cheat sheet
  ...
  - [x] Task 31.2. Render it to PDF via headless Chrome (`--headless --print-to-pdf --no-pdf-header-footer`),
        ...
        in).
  - [x] Task 31.3. Create `modules/ROOT/pages/programming-languages/c/cheat-sheet.adoc` — `:description:`/
...
- [x] Task 32. Create `modules/ROOT/pages/programming-languages/c/index.adoc` — "C Reference" landing page
      (mirrors `programming-languages/kotlin/index.adoc`):
  - [x] Task 32.1. Title, `:description:`, `:keywords:` (including `C, C23, ISO/IEC 9899:2024`),
  ...
  - [x] Task 32.2. `== What's covered` — full bulleted list of Tasks 2–30 + Task 31 (cheat sheet), grouped under
        ...
        Tooling and standards.
  - [x] Task 32.3. `== Bibliography` section, anchor `_bibliography`, listing every source from the issue,
        ...
        discrepancy.
- [x] Task 33. Wire navigation and disambiguate the include site:
  - [x] Task 33.1. Create `modules/ROOT/partials/nav-c.adoc`, written exactly like `nav-kotlin.adoc` (its own
        ...
        (PDF)]`.
  - [x] Task 33.2. In `modules/ROOT/nav.adoc`, add `include::partial$nav-c.adoc[]` on its own line immediately
...
- [x] Task 34. Update `modules/ROOT/pages/programming-languages/index.adoc`:
  - [x] Task 34.1. Add a sixth `== Sections` bullet, `xref:programming-languages/c/index.adoc[C Reference]`,
  ...
  - [x] Task 34.2. Refresh the page's `:description:`/`:keywords:` to mention C/C23.
- [x] Task 35. Update `modules/ROOT/pages/index.adoc`'s `:keywords:` line (line 3) to add `C, C23` (matching the
...
- [x] Task 36. Build verification — delegate to a sub-agent so Antora's build output doesn't consume the main
      context window:
      ```
      Agent({
