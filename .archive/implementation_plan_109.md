...
- [x] Task 2. `getting-started.adoc` — "Getting Started"
  - [x] Task 2.1. What Objective-C is (C plus Smalltalk-style messaging); short history (Cox & Love → NeXT →
        ...
        `.h`/`.m`/`.mm`.
  - [x] Task 2.2. "Hello, World" with `#import <Foundation/Foundation.h>`, `@autoreleasepool` and `NSLog`, plus
        command-line build/run lines for both `clang -fobjc-arc -framework Foundation` and
        `` `gnustep-config --objc-flags` ``.
  - [x] Task 2.3. Add `modules/ROOT/images/objective-c-compilation-pipeline.svg` — preprocess → compile →
...
- [x] Task 3. `basic-syntax-and-types.adoc` — "Basic Syntax and Types"
  - [x] Task 3.1. Tokens, comments, identifiers, keywords and the `@`-directives table; the C scalar types in
  ...
  - [x] Task 3.2. Variables and constants, literals, `typedef`, `NS_ENUM`/`NS_OPTIONS`, casts and the usual
        arithmetic conversions — **summarised**, with the C detail deferred (see choice 2 / Task 31).
- [x] Task 4. `operators-control-flow-and-functions.adoc` — "Operators, Control Flow and Functions"
  - [x] Task 4.1. Operators and precedence, `if`/`else`, `switch`, the loop forms, the ternary operator — each
        briefly, Objective-C-specific notes (e.g. `BOOL` truthiness, messaging inside conditions) foregrounded.
  - [x] Task 4.2. C functions vs. Objective-C methods, pointers, `struct`/`union`, and the preprocessor as used
        in Objective-C: `#import` vs. `#include`, `#pragma mark`, `#if TARGET_OS_*`.

### Group 3 — Objects, classes and messaging (Parallelizable: yes)

- [x] Task 5. `classes-and-objects.adoc` — "Classes and Objects"
  - [x] Task 5.1. `@interface`/`@implementation`, instance variables and visibility
  ...
  - [x] Task 5.2. `alloc`/`init`, designated and convenience initializers (`NS_DESIGNATED_INITIALIZER`,
        `instancetype`, `new`), `self` and `super`, `dealloc`, `NSObject` as root class and the `isa` pointer,
        `description`.
  - [x] Task 5.3. Add `modules/ROOT/images/objective-c-interface-implementation-anatomy.svg` — a labelled
        `.h`/`.m` pair showing which declaration lives where — and include it.
- [x] Task 6. `messaging-and-selectors.adoc` — "Messaging and Selectors"
  - [x] Task 6.1. Message-send syntax, nested messages, **messages to `nil`** and what they return; selectors
        (`@selector`, `SEL`, `NSSelectorFromString`), `respondsToSelector:`, `performSelector:`, `IMP`.
  - [x] Task 6.2. Dynamic binding and polymorphism; dynamic vs. static typing (`id` vs. typed pointers,
        `__kindof`); introspection (`isKindOfClass:`, `isMemberOfClass:`, `class`, `conformsToProtocol:`).
  - [x] Task 6.3. Add a `[mermaid]` flowchart of how a message send is resolved (cache → method list →
...
- [x] Task 7. `properties-and-encapsulation.adoc` — "Properties and Encapsulation"
  - [x] Task 7.1. `@property` and the full attribute table: `strong`/`weak`/`copy`/`assign`/`unsafe_unretained`,
        `atomic`/`nonatomic`, `readonly`/`readwrite`, `getter=`/`setter=`, `class`, nullability.
  - [x] Task 7.2. Autosynthesis, `@synthesize` and `@dynamic`, backing ivars (`_name`), dot syntax vs. accessor
...
- [x] Task 8. `inheritance-and-polymorphism.adoc` — "Inheritance and Polymorphism"
  - [x] Task 8.1. Subclassing, overriding and calling `super`, abstract classes and
        `doesNotRecognizeSelector:`, `isEqual:`/`hash` contract, `NSCopying`/`NSMutableCopying`.
  - [x] Task 8.2. Class clusters (`NSString`, `NSArray`, `NSNumber`) and composite objects; when to subclass vs.
        compose.
  - [x] Task 8.3. Add a `[mermaid]` class diagram of a small hierarchy illustrating the above.
- [x] Task 9. `categories-and-extensions.adoc` — "Categories and Extensions"
  - [x] Task 9.1. Categories (adding methods to existing classes, splitting large classes), class extensions
        (the private interface), associated objects (`objc_setAssociatedObject`/`objc_getAssociatedObject`).
  - [x] Task 9.2. The category pitfalls (no ivars, name collisions, overriding is undefined-order) and **posing
        (`poseAsClass:`) documented as a removed legacy feature only**.
- [x] Task 10. `protocols-and-delegation.adoc` — "Protocols and Delegation"
  - [x] Task 10.1. `@protocol`, `@required`/`@optional`, adopting and conforming, `id<Protocol>` typing,
        protocol inheritance, the `NSObject` protocol, run-time conformance checks.
  - [x] Task 10.2. The delegate and data-source patterns and why delegates are `weak`.
  - [x] Task 10.3. Add a `[mermaid]` sequence diagram of a delegate callback.
- [x] Task 11. `blocks.adoc` — "Blocks"
  - [x] Task 11.1. Block syntax and `typedef`s, blocks as method arguments and completion handlers, capturing
        variables and `__block`, stack vs. heap blocks and copying.
  - [x] Task 11.2. Retain cycles and the `__weak`/`__strong self` dance, `NSArray` block enumeration, blocks vs.
        C function pointers.
  - [x] Task 11.3. Add `modules/ROOT/images/objective-c-block-capture.svg` — variable capture and the stack→heap
        copy — and include it.

### Group 4 — Foundation values and collections (Parallelizable: yes)

- [x] Task 12. `strings-numbers-and-values.adoc` — "Strings, Numbers and Values"
  - [x] Task 12.1. `NSString`/`NSMutableString` (literals `@"…"`, formatting, comparison, ranges, encoding
        conversions); `NSNumber` and boxing literals (`@42`, `@YES`, `@(expr)`); `NSValue`.
  - [x] Task 12.2. `NSData`, `NSDate`, `NSNull`, `NSURL`; `NSString` vs. C strings; toll-free bridging to
        CoreFoundation (`CFStringRef`), forward-referencing the ARC bridging casts page.
- [x] Task 13. `collections-and-fast-enumeration.adoc` — "Collections and Fast Enumeration"
  - [x] Task 13.1. `NSArray`/`NSDictionary`/`NSSet` and their mutable variants; collection literals (`@[]`,
        `@{}`) and subscripting; immutability and copying semantics.
  - [x] Task 13.2. Fast enumeration (`for…in`), `NSEnumerator`, block-based enumeration, sorting and filtering
        (`NSPredicate`, comparators), `NSIndexSet`, `NSCache`, collection thread-safety.
  - [x] Task 13.3. Add a `[mermaid]` class diagram of the Foundation collection hierarchy.
- [x] Task 14. `lightweight-generics-and-nullability.adoc` — "Lightweight Generics and Nullability"
  - [x] Task 14.1. `NSArray<NSString *> *`, generic class declarations (`__covariant`/`__contravariant`),
        `__kindof`.
  - [x] Task 14.2. `NS_ASSUME_NONNULL_BEGIN`/`END`, `nullable`/`nonnull`/`null_resettable`/`null_unspecified`
...
- [x] Task 15. `manual-retain-release.adoc` — "Manual Retain Release"
  - [x] Task 15.1. The ownership model and retain counts, the four rules (`alloc`/`new`/`copy`/`mutableCopy`
        own; `retain`/`release`/`autorelease`), autorelease pools, accessor patterns under MRR, `dealloc`.
  - [x] Task 15.2. The classic bugs (leaks, over-release, dangling pointers). Keep the page **short** — ARC is
        the default; this page exists to read older code — and link forward to
        `automatic-reference-counting.adoc`.
  - [x] Task 15.3. Add a `[mermaid]` diagram of an object's lifecycle under MRR.
- [x] Task 16. `automatic-reference-counting.adoc` — "Automatic Reference Counting"
  - [x] Task 16.1. What ARC does and does not do; ownership qualifiers (`__strong`, `__weak`,
        `__unsafe_unretained`, `__autoreleasing`); method families and naming conventions
        (`NS_RETURNS_RETAINED` etc.).
  - [x] Task 16.2. Retain cycles and weak references (delegates, blocks, parent/child), `dealloc` under ARC,
        `@autoreleasepool` in loops.
  - [x] Task 16.3. Bridging casts with CoreFoundation (`__bridge`, `__bridge_retained`, `__bridge_transfer`),
        `-fobjc-arc`/`-fno-objc-arc` per file, diagnosing leaks with Instruments and the static analyzer.
  - [x] Task 16.4. Add `modules/ROOT/images/objective-c-retain-cycle.svg` — a retain cycle and how `weak` breaks
        it — and include it.
- [x] Task 17. `the-objective-c-runtime.adoc` — "The Objective-C Runtime"
  - [x] Task 17.1. `objc_msgSend` and the dispatch path; classes, metaclasses and `isa`; method lists and
        caches.
  - [x] Task 17.2. The runtime API (`objc_getClass`, `class_addMethod`, `class_copyMethodList`,
        ...
        `NSProxy`.
  - [x] Task 17.3. Add `modules/ROOT/images/objective-c-class-metaclass-isa.svg` — the class / metaclass /
        superclass `isa` diagram — and include it.
- [x] Task 18. `dynamic-method-resolution-and-forwarding.adoc` — "Dynamic Method Resolution and Forwarding"
  - [x] Task 18.1. `+resolveInstanceMethod:`/`+resolveClassMethod:` (with `@dynamic` properties),
  ...
  - [x] Task 18.2. Add a `[mermaid]` diagram of the full message-forwarding chain (continuing the figure from
        Task 6.3).
- [x] Task 19. `key-value-coding-and-observing.adoc` — "Key-Value Coding and Observing"
  - [x] Task 19.1. KVC: `valueForKey:`, `setValue:forKey:`, key paths, collection operators (`@sum`, `@avg`),
        validation, `NSKeyValueCoding` compliance.
  - [x] Task 19.2. KVO: `addObserver:forKeyPath:options:context:`, `observeValueForKeyPath:…`, automatic vs.
  ...
  - [x] Task 19.3. Add a `[mermaid]` sequence diagram of a KVO notification.

### Group 6 — Error handling, concurrency and interoperability (Parallelizable: yes)

- [x] Task 20. `errors-and-exceptions.adoc` — "Errors and Exceptions"
  - [x] Task 20.1. The `NSError` pattern: `NSError **` out-parameters, the return-value-then-error discipline,
        domains/codes/`userInfo`, creating and propagating errors, `NSErrorDomain`/`NS_ERROR_ENUM`.
  - [x] Task 20.2. Exceptions (`@try`/`@catch`/`@finally`/`@throw`, `NSException`, uncaught-exception handlers)
  ...
  - [x] Task 20.3. Add a `[mermaid]` decision diagram: choosing between `NSError` and an exception.
- [x] Task 21. `concurrency.adoc` — "Concurrency"
  - [x] Task 21.1. Threads (`NSThread`, the main thread); synchronisation (`@synchronized`, `NSLock`, atomic
        properties and **why `atomic` is not thread safety**).
  - [x] Task 21.2. GCD (`dispatch_async`, serial/concurrent queues, `dispatch_group`, `dispatch_semaphore`,
  ...
  - [x] Task 21.3. Add a `[mermaid]` sequence diagram: background work dispatched back to the main queue.
- [x] Task 22. `objective-c-plus-plus-and-c-interop.adoc` — "Objective-C++ and C Interoperability"
  - [x] Task 22.1. Objective-C++ (`.mm` files, C++ objects as ivars, calling C++ from methods, C++11 lambdas
        vs. blocks, ARC and C++ containers).
  - [x] Task 22.2. Calling plain C libraries, `extern "C"` and header hygiene, CoreFoundation and toll-free
        bridging, wrapping opaque C types in Objective-C classes.
- [x] Task 23. `swift-interoperability.adoc` — "Swift Interoperability"
  - [x] Task 23.1. The bridging header, the generated `<Module>-Swift.h` header, `@objc`/`@objcMembers` on the
        Swift side, `NS_SWIFT_NAME`, `NS_REFINED_FOR_SWIFT`, `NS_SWIFT_UNAVAILABLE`.
  - [x] Task 23.2. How nullability, lightweight generics, `NS_ENUM`/`NS_OPTIONS`, error out-parameters and
        `instancetype` are imported into Swift; mixed-language projects.
  - [x] Task 23.3. Add `modules/ROOT/images/objective-c-swift-interop.svg` — how the two languages see each
        other within one target — and include it.

### Group 7 — Code organisation and tooling (Parallelizable: yes)

- [x] Task 24. `modules-frameworks-and-code-organization.adoc` — "Modules, Frameworks and Code Organization"
  - [x] Task 24.1. `#import` vs. `#include` vs. `@import`; Clang modules and `module.modulemap`; umbrella
        headers; static vs. dynamic frameworks; public/private/project header visibility.
  - [x] Task 24.2. `NS_UNAVAILABLE` and `NS_DESIGNATED_INITIALIZER` in public APIs, `@compatibility_alias`,
        organising a class into files and `#pragma mark` sections, class prefixes as namespaces-by-convention.
  - [x] Task 24.3. Add a `[mermaid]` diagram of a framework bundle's anatomy.
- [x] Task 25. `coding-conventions-and-style.adoc` — "Coding Conventions and Style"
  - [x] Task 25.1. Apple's Coding Guidelines for Cocoa (method, accessor, delegate, constant and notification
        naming; prefixes) and the "Conventions" chapter of *Programming with Objective-C*.
  - [x] Task 25.2. Documentation comments (`///`, `/** */`, HeaderDoc / Xcode Quick Help), deprecation and
        availability attributes.
  - [x] Task 25.3. The Cocoa design patterns every Objective-C programmer meets: delegation, target-action,
        notifications, singleton via `dispatch_once`, MVC and its MVP/MVVM variants.
- [x] Task 26. `build-and-tooling.adoc` — "Build and Tooling"
  - [x] Task 26.1. Xcode and `xcodebuild`; Apple Clang flags (`-fobjc-arc`, `-fmodules`, `-ObjC`,
        `-Wall -Wextra`, `-Werror`); building on Linux with GCC/GNUstep (`gnustep-config --objc-flags`) and on
        Windows.
  - [x] Task 26.2. The Clang Static Analyzer (`xcodebuild analyze`), sanitizers (Address/Thread/Undefined),
        Instruments (Leaks, Allocations, Zombies), debugging with `lldb` (`po`, `expr`, breakpoints, exception
        breakpoints).
  - [x] Task 26.3. Dependency managers (CocoaPods, Swift Package Manager with Objective-C targets) and
        `clang-format`.
- [x] Task 27. `testing.adoc` — "Testing"
  - [x] Task 27.1. XCTest: `XCTestCase`, `setUp`/`tearDown`, the `XCTAssert…` family, `XCTestExpectation` for
        asynchronous code, performance tests with `measureBlock:`.
  - [x] Task 27.2. Mocking with OCMock (link its official site), running tests with `xcodebuild test`, and
...
- [x] Task 28. `index.adoc` — "Objective-C Reference" landing page
  - [x] Task 28.1. Title/description/keywords + disclaimer include; one paragraph on what Objective-C is; a
        "New here? read in this order" paragraph (Getting Started → the C-level pages → Objects/classes/
        messaging → the rest as needed).
  - [x] Task 28.2. `== What's covered` — one bullet per page with a one-line summary, grouped under the **same
  ...
  - [x] Task 28.3. `== Bibliography` — every source listed in issue #109's Bibliography section, **all linked**:
        ...
        source).
- [x] Task 29. `cheat-sheet.adoc` — "Objective-C Cheat Sheet"
  - [x] Task 29.1. Short description of what the sheet covers, mirroring
        `programming-languages/kotlin/cheat-sheet.adoc`'s structure.
  - [x] Task 29.2. Grouped cross-references to **every** topic page (same grouping as Task 28.2).
  - [x] Task 29.3. The download link:
...
- [x] Task 30. Produce `modules/ROOT/attachments/objective-c-cheat-sheet.pdf`
  - [x] Task 30.1. Author a throwaway print-ready A4 HTML/CSS layout (scratch directory, **not** checked in),
  ...
  - [x] Task 30.2. Cover, per issue #109: Objective-C types (`id`, `BOOL`, `SEL`, `nil`),
  ...
  - [x] Task 30.3. Render with headless Chromium (pre-installed at `/opt/pw-browsers/chromium` — do **not** run
  ...
  - [x] Task 30.4. Verify it is **exactly one A4 page** with no clipped content (page-count check plus a
...
- [x] Task 31. Nav partial and `nav.adoc` include
  - [x] Task 31.1. Create `modules/ROOT/partials/nav-objective-c.adoc` modelled exactly on
  ...
  - [x] Task 31.2. Include it in `modules/ROOT/nav.adoc` **inside the same open block (`--`) as
...
- [x] Task 33. Extend the root `modules/ROOT/pages/index.adoc` keywords
  - [x] Task 33.1. Append `Objective-C, Cocoa, Foundation` to the `:keywords:` line (line 3), preserving the
...
- [x] Task 34. Resolve the C Reference cross-reference decision (see choice 2)
  - [x] Task 34.1. Check whether `modules/ROOT/pages/programming-languages/c/` exists in the branch at this
        point (i.e. PR #108 has merged and been brought in).
  - [x] Task 34.2. **If it exists**: replace the self-contained C-level summaries in
...
- [x] Task 35. Build and verify the site
  ...
  - [x] Task 35.2. Confirm **zero** `xref`/AsciiDoc warnings or errors — in particular no unresolved
  ...
  - [x] Task 35.3. Spot-check the generated `build/site`: the Objective-C Reference appears in the nav under
  ...
  - [x] Task 35.4. Walk issue #109's eight acceptance criteria one by one and confirm each, noting explicitly
