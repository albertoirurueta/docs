...
- [x] Task 1. Create `modules/ROOT/pages/backend/graphql/when-to-use-graphql-or-rest.adoc` — page written from
      issue #102's body/comment, all 13 sub-tasks below completed, re-checked against all 22 acceptance criteria.
  ...
  - [x] Task 1.1. Page header: `= Tips: When to Use GraphQL or a Typical REST API`, `:description:` (one
  ...
  - [x] Task 1.2. `== What each model actually gives you` — the single typed graph vs. URL-addressed resources,
  ...
  - [x] Task 1.3. `== Two meanings of "performance"` — server-side cost/predictability vs. end-to-end
  ...
  - [x] Task 1.4. `== Client-selectable access paths and query-plan cost` — the new core section, covering (per
  ...
  - [x] Task 1.5. `== Pros and cons at a glance` — the full draft table from the issue (23 rows), using the
  ...
  - [x] Task 1.6. `== Where GraphQL clearly wins` — bullet list per the issue: many heterogeneous clients evolving
  ...
  - [x] Task 1.7. `== Where REST (or gRPC) is the better answer` — bullet list per the issue: latency-critical hot
        ...
        REST one.
  - [x] Task 1.8. `== The two deployment profiles` — the internal/backoffice vs. public/partner profile table from
  ...
  - [x] Task 1.9. `== Tips for running both side by side` — all 29 tips from the issue, grouped exactly as
        ...
        degradation).
  - [x] Task 1.10. `== Decision checklist` — the full 15-row scenario -> recommendation table from the issue.
  - [x] Task 1.11. `== Related pages` — bullet list of xrefs (unquoted `xref:`, matching `serving-over-http.adoc`'s
  ...
  - [x] Task 1.12. `== Further reading` — the 16 external references from the issue's "References" section as a
  ...
  - [x] Task 1.13. Re-read the drafted page end-to-end against the issue's 22 acceptance-criteria checkboxes (the
...
- [x] Task 2. Add nav entry in `modules/ROOT/nav.adoc` — inserted between `federation.adoc` and
      `spring-boot-getting-started.adoc`, no other line touched.
  - [x] Task 2.1. Insert a new `****`-level line
...
- [x] Task 3. Add a new group to `modules/ROOT/pages/backend/graphql/index.adoc` — new `=== Choosing between
      GraphQL and REST` group inserted after the `federation.adoc` bullet, before `=== Spring Boot integration`.
  - [x] Task 3.1. Insert a new `=== Choosing between GraphQL and REST` group immediately after the
...
- [x] Task 4. Forward link from `modules/ROOT/pages/backend/graphql/getting-started.adoc`
  - [x] Task 4.1. At the paragraph ending "...a typed schema enforced by the server." (immediately before
...
- [x] Task 5. Link from `modules/ROOT/pages/backend/graphql/security-and-demand-control.adoc`
  - [x] Task 5.1. Add one bullet to the existing `== Further reading` section (around lines 282-292), following
...
- [x] Task 6. Link from `modules/ROOT/pages/backend/graphql/caching.adoc`
  - [x] Task 6.1. This page has no dedicated closing section — add one clause with an inline
...
- [x] Task 7. Link from `modules/ROOT/pages/backend/graphql/serving-over-http.adoc`
  - [x] Task 7.1. Add one bullet to the existing `== Related pages` section (around lines 336-347), following
...
- [x] Task 8. Verify the Antora build — `npx antora antora-playbook.yml` completed with exit code 0 and no
      xref/AsciiDoc errors or warnings; `build/site/backend/graphql/when-to-use-graphql-or-rest.html` rendered.
  - [x] Task 8.1. Ran the build (no `iru-gate-runner`/`Agent` tool was available in this execution environment, so
        run directly rather than via a delegated sub-agent) — clean, empty build log, exit 0.
  - [x] Task 8.2. No `xref`/AsciiDoc error was reported — nothing to fix.
